...
class VivaScreen(QWidget):
    def __init__(self, parent=None):
        ...
        # ================= SECTION 1: OFFLINE VIVA (TOP) =================
        self.offline_card = CardFrame(padding=0) # We will manage margins inside
        ...
        off_title_lbl.setStyleSheet(f"padding: 20px; color: {Theme.text_primary}; border-bottom: 1px solid {Theme.border};")
        offline_v.addWidget(off_title_lbl)
        
        # Split Layout for Offline
        self.splitter = QSplitter(Qt.Orientation.Horizontal)
        self.splitter.setStyleSheet("QSplitter::handle { background-color: #e5e7eb; width: 1px; }")
        ...
        self.form_instruct.setStyleSheet(f"color: {Theme.text_muted}; margin-top: 50px;")
        ...
        self.marks_input.setStyleSheet(f"border: 1px solid {Theme.border}; border-radius: 8px; padding: 10px;")
        
        self.feedback_input = QTextEdit()
        self.feedback_input.setPlaceholderText("Feedback textarea")
        self.feedback_input.setMinimumHeight(150)
        self.feedback_input.setStyleSheet(f"border: 1px solid {Theme.border}; border-radius: 8px; padding: 10px;")
        
        self.save_btn = QPushButton("Save & Next Student")
        self.save_btn.setStyleSheet(f"""
            QPushButton {{ background: {Theme.success}; color: white; border-radius: 8px; padding: 12px; font-weight: bold; font-size: 14px; }}
        ...
        self.publish_offline_btn.setStyleSheet(f"""
            QPushButton {{ background: {Theme.primary}; color: white; border-radius: 8px; padding: 12px; font-weight: bold; font-size: 14px; }}
        ...
        on_title_lbl.setStyleSheet(f"color: {Theme.text_primary}; margin-bottom: 20px;")
        ...
        self.publish_btn.setStyleSheet(f"""
            QPushButton {{ background: {Theme.primary}; color: white; border-radius: 8px; padding: 15px; font-weight: bold; font-size: 15px; margin-top: 10px; }}
            QPushButton:hover {{ background: #1d4ed8; }}
        """)
    ...
    def hideEvent(self, event):
        """Clean up WebSocket when screen is hidden"""
        if self.ws_client:
            self.ws_client.stop()
            self.ws_client = None
    ...
    def sync_offline_session(self):
        """Find or create an offline session for today for this batch"""
        ...
        # 1. ALWAYS load students first so UI isn't empty!
        self.load_records()

        # Initialize WebSocket for monitoring if not already
        ...
        # Check if an offline session already exists for today
        try:
            res = api_client.get(f"viva-sessions/?batch={batch_id}&viva_type=offline")
            if res.status_code == 200:
                sessions = res.json()
                if sessions:
                    self.current_viva_session_id = sessions[0]['id']
                    self.load_records() # Load records now that we have session ID
                else:
    ...
    def handle_status_update(self, data):
        """Toggle online/offline dot for students"""
    ...
    def load_records(self):
        ...
        # Mandatory GET /api/students/?batch=<id>
        try:
        ...
        if self.current_viva_session_id:
            try:
                res = api_client.get(f"viva/?viva_session={self.current_viva_session_id}")
                if res.status_code == 200:
                    data = res.json()
                    # Handle paginated response
                    if isinstance(data, dict) and 'results' in data:
                        self.viva_records = data['results']
                    elif isinstance(data, list):
                        self.viva_records = data
                    else:
                        self.viva_records = []
            except Exception as e:
    ...
    def add_student_item(self, student, rec):
        ...
        # Default to offline until update comes or we can fetch initial
        presence_dot.setStyleSheet(f"background: #D1D5DB; border-radius: 5px;")
        ...
        name_lbl.setWordWrap(True) # No truncation
        ...
        badge = QLabel("Not Evaluated" if status == 'waiting' else "Evaluated ✓")
        badge_color = "#EAB308" if status == 'waiting' else "#10B981"
    ...
    def _ensure_offline_session(self) -> bool:
        """Auto-create the offline session if it wasn't created on startup. Returns True if session ID is ready."""
        if self.current_viva_session_id:
            return True
        ...
        batch_id = self.parent_window.current_batch_id
        try:
            # Try to find an existing offline session
            res = api_client.get(f"viva-sessions/?batch={batch_id}&viva_type=offline")
            if res.status_code == 200:
                data = res.json()
            ...
            if create_res.status_code == 201:
                self.current_viva_session_id = create_res.json()['id']
                return True
            QMessageBox.critical(self, "Error", f"Could not create offline session: {create_res.text}")
        except Exception as e:
    ...
    def save_evaluation(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Error", "Invalid marks")
            return
            
        feedback = self.feedback_input.toPlainText().strip()
        
        # POST update as required
        ...
        if res.status_code in (200, 201):
            ...
            # Update the UI badge instantly for the current item
            ...
            if current_item:
                ...
                if badge:
                    ...
                    badge.setStyleSheet(f"background: #10B981; color: white; border-radius: 4px; padding: 4px 10px; font-size: 11px; font-weight: bold;")
            ...
            if current_idx < self.student_list.count() - 1:
                ...
                QMessageBox.information(self, "Done", "All students evaluated. Remember to publish results!")
        else:
            QMessageBox.critical(self, "Error", f"Failed to save evaluation: {res.text}")

    def publish_offline_results(self):
        if not self.current_viva_session_id: return
        res = api_client.post(f"viva-sessions/{self.current_viva_session_id}/complete/")
        if res.status_code == 200:
            QMessageBox.information(self, "Success", "Offline Viva results published to all students.")
        else:
            QMessageBox.critical(self, "Error", "Failed to publish Offline Viva results.")

    def publish_online_viva(self):
        if not hasattr(self.parent_window, 'current_batch_id') or self.parent_window.current_batch_id is None:
            QMessageBox.warning(self, "Error", "Select batch first")
            return
        ...
        data = {
            ...
            "start_time": self.start_time_input.dateTime().toPyDateTime().isoformat(),
            "instructions": self.on_instructions.toPlainText()
        }
        
        # 1. Create and Publish Session in one go
        res = api_client.post("viva-sessions/", data)
        if res.status_code == 201:
            QMessageBox.information(self, "Success", "Online Viva published & visible to students!")
        else:
            QMessageBox.critical(self, "Error", f"Failed: {res.text}")
